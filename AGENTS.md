...
`creators` → `curated_channels` → `channels` → `videos`; `user_subscriptions` joins users to creators; `watch_sessions` tracks per-user playback.
...
|-------|--------|--------------------------|
| `creators`, `channels`, `curated_channels`, `videos`, `channel_calibration`, `sync_queue` | any authenticated | admin only |
